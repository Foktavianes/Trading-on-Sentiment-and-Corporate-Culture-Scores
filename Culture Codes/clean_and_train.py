...
clean_file(
    in_file=Path(global_options.DATA_FOLDER, "processed", "parsed", "CombinedQnA.txt"),  #Change this
    out_file=Path(global_options.DATA_FOLDER, "processed", "unigram", "CombinedQnA.txt"), #Change this
)
...
culture_models.train_bigram_model(
    input_path=Path(
        global_options.DATA_FOLDER, "processed", "unigram", "CombinedQnA.txt"
    ),
...
culture_models.file_bigramer(
    ...
    output_path=Path(
        global_options.DATA_FOLDER, "processed", "bigram", "CombinedQnA.txt"
    ),
    model_path=Path(global_options.MODEL_FOLDER, "phrases", "bigram.mod"),
...
culture_models.train_bigram_model(
    input_path=Path(global_options.DATA_FOLDER, "processed", "bigram", "CombinedQnA.txt"),
    model_path=Path(global_options.MODEL_FOLDER, "phrases", "trigram.mod"),
)
culture_models.file_bigramer(
    input_path=Path(global_options.DATA_FOLDER, "processed", "bigram", "CombinedQnA.txt"),
    output_path=Path(
        global_options.DATA_FOLDER, "processed", "trigram", "CombinedQnA.txt"
    ),
    model_path=Path(global_options.MODEL_FOLDER, "phrases", "trigram.mod"),
    scoring="original_scorer",
    threshold=global_options.PHRASE_THRESHOLD,
