...
if __name__ == "__main__":
    with CoreNLPClient(
        ...
        in_file = Path(global_options.DATA_FOLDER, "input", "2019.txt")    #Change this to text data file name
        in_file_index = file_util.file_to_list(
            Path(global_options.DATA_FOLDER, "input", "2019_id.txt")       #Change accordingly
        )
        out_file = Path(
            global_options.DATA_FOLDER, "processed", "parsed", "2019.txt"  #Change name for identification
        )
        output_index_file = Path(
            global_options.DATA_FOLDER, "processed", "parsed", "2019_id.txt"  #Change name for identification
        )
        process_largefile(
            input_file=in_file,
